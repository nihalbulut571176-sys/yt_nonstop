...
def create_project_manifest(
    ...
    now = iso_now()
    manifest["profile_id"] = "vlm_unstop"
    ...
    manifest["prompts"]["generator_ready_path"] = str(dirs["prompts"] / "vlm_prompts_generator_ready.md")
    manifest["prompts"]["prompt_review_path"] = str(dirs["prompts"] / "prompt_review.md")

    manifest["images"]["status"] = "skipped"
    manifest["images"]["provider"] = None
    manifest["images"]["route"] = None
    ...
    manifest["images"]["generated_count"] = 0
    manifest["images"]["failed_count"] = 0

    manifest["animation"]["status"] = "pending"
    manifest["animation"]["policy_name"] = "all_scenes_text_to_video"
