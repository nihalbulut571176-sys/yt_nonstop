...
def build_stage_command(project_json: Path, stage: str, args: argparse.Namespace) -> list[str]:
    if stage == "transcription":
    ...
    if stage == "export_prompts":
        cmd = [sys.executable, str(ROOT / "scripts" / "export_project_vlm_prompts.py"), "--project-json", str(project_json)]
    ...
    if stage == "generate_videos":
        cmd = [
            sys.executable,
            str(ROOT / "scripts" / "run_project_vlm_video_generation.py"),
            "--project-json",
            str(project_json),
            "--aspect-ratio",
            args.aspect_ratio,
...
def main() -> None:
    ...
    try:
        for stage in selected_stages:
            ...
            cmd = build_stage_command(project_json, stage, args)
